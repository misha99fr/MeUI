...
local QRCode = require("TabletOSGraphics/QRCode").qrcode
local braile = require("TabletOSGraphics/braile")
local graphics = {
  theme = {
    bars = {
      foreground = 0xFFFFFF,
    ...
    qrCode = {
      ...
      frame = 0x000000,
      titleBackground = 0x888888,
      titleForeground = 0xFFFFFF,
      buttonBack = 0x555555,
      buttonFore = 0xFFFFFF,
      commonBackground = 0x888888,
    },
  },
}
...
function graphics.drawQRCodeWindow(string,...) --uses braileMatrix
  local success, matrix = QRCode(string,...) --поле чисел, преобразование в boolean - matrix[x][y] > 0
  local sW, sH = buffer.getResolution()
  local w, h = math.ceil(#matrix/2)+2,math.ceil(#matrix[1]/4)+2
  local x,y = (sW-w)/2+1,(sH-h)/2+1
  x,y,w,h = math.floor(x+0.5),math.floor(y+0.5),math.floor(w+0.5),math.floor(h+0.5)
  local screen = buffer.copy(x,y,w,h)
  local theme = graphics.theme.qrCode
...
--local checkTouch = graphics.drawButton(x,y+h-1,w,1,core.getLanguagePackages().OS_close,graphics.theme.infoWindow.buttonBack,graphics.theme.infoWindow.buttonFore)
  ...
  do
    for x = 1, #matrix do
      for y = 1, #matrix[x] do
        braile.set(braileMatrix,x,y,matrix[x][y] > 0 and 1 or 0)
      end
    end
    braile.render(braileMatrix,x+1,y+1,theme.black_pixel,theme.white_pixel)
  end
  graphics.drawChanges()
  while true do
    local event, _, tX, tY = event.pull(0.5)
    if (event == "touch" and not graphics.clickedAtArea(x,y,x+w-1,y+h-1,tX,tY)) or (event == "drop" and close(tX,tY)) then
      buffer.paste(x,y,screen)
      graphics.drawChanges()
      break
    end
